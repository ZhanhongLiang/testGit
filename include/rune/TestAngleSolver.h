...
struct AngleSolverParam
{
	...
	cv::Size CAM_SIZE = cv::Size(720, 480);
...
public:
	...
	*/
	void setTarget1(const std::vector<cv::Point2f> objectPoints, int objectType);  //set corner points for PNP
	...
	void /*AngleSolver::*/getTarget2dPoinstion(const cv::RotatedRect & rect, vector<cv::Point2f> & target2d, const cv::Point2f & offset);
	//void getTarget2dPoinstion(const cv::RotatedRect & rect, vector<cv::Point2f> & target2d, const cv::Point2f & offset);
																  /*
	...
	*/
	...
	vector<cv::Point2f> point_of_armor;
...
public:
	...
	cv::Size2i image_size = cv::Size2i(760, 480);
	int user_type = 1;
	int enemy_type = 1;
